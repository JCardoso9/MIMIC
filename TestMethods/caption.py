...
def main():
    ...
    refs_path, preds_path = save_references_and_predictions(references, hypotheses, argParser.model_name)

    with open('../Experiments/' + argParser.model_name + "/TestResults.txt", "w+") as file:
      for metric in metrics_dict:
        file.write(metric + ":" + str(metrics_dict[metric]) + "\n")

def evaluate(argParser, beam_size, encoder, decoder, testLoader, word2idx, idx2word):
    """
    Evaluation
